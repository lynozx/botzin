...
const config = {
    XBOT: '𝐋𝐘𝐍𝐎 𝐁𝐎𝐓ϟ', 
    instagram: 'https://instagram.com/offdezin', 
    nomer: 'wa.me/558897187831',
    youtube: 'https://youtube.com/channel/UC2t4WHwlZRVgYpeYvqtNgTQ', 
		...
		try {
			...
			switch(command) {
				...
				case 'info':
					...
					break
					case '(bot)':
		if (args.length < 1) return reply ('(opa)')
		break
				case 'blocklist':
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						mentions(`parabens @${mentioned[0].split('@')[0]} já virou adm kkk`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`perdeu adm @${mentioned[0].split('@')[0]} 😔`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`vai de ban : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'simi':
					...
					//if (anu.error) return reply('sei nn men')
					reply(anu)
					break
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
